...
При обновлении поверх 1.4.3 запуск автоматически удаляет только восемь
известных устаревших исходников (`core/loader.py`, `inline/core.py`,
`modules/backup.py`, старые `utils.py`/`utils_additions.py` и старый пакет
`secure`). Конфиг, сессии, БД, пользовательские модули и неизвестные локальные
файлы не затрагиваются.

Рабочий startup подтверждается строкой
`Telethon update loop scheduled (run_until_disconnected)` в журнале. Догон
пропущенных событий (`catch_up`) запускается отдельно и не блокирует живые
команды. Если update loop аварийно остановится, процесс теперь завершит startup
с настоящей ошибкой Telethon вместо внешне рабочей панели при молчащих командах.
