...
def _ec_const(ec) -> int:
    if isinstance(ec, int):
        return ec
    key = str(ec or "M").strip().upper()
    return _EC_MAP.get(key, ERROR_CORRECT_M)


def _matrix_bool(text: str, ec: str = "M") -> list[list[bool]]:
    qr = QRCode(error_correction=_ec_const(ec), border=0)
    qr.add_data(text)
    qr.make(fit=True)
    return [[bool(cell) for cell in row] for row in qr.get_matrix()]


def _grid_with_quiet(rows, quiet: int) -> list[list[int]]:
    quiet = max(0, int(quiet))
    body = [[1 if cell else 0 for cell in row] for row in rows]
    n = len(body[0]) if body else 0
    width = n + quiet * 2
    grid: list[list[int]] = [[0] * width for _ in range(quiet)]
    for row in body:
        grid.append([0] * quiet + row + [0] * quiet)
...
def make_qr_text_compact(text: str, ec: str = "M", quiet: int = 4) -> str:
    grid = _grid_with_quiet(_matrix_bool(text, ec), quiet)
    if len(grid) % 2:
    ...
    for i in range(0, len(grid), 2):
        top = grid[i]
        bot = grid[i + 1]
        lines.append("".join(_half_block(1 - t, 1 - b) for t, b in zip(top, bot)))
    return "\n".join(lines)


def make_qr_text_full(text: str, ec: str = "M", quiet: int = 4) -> str:
    grid = _grid_with_quiet(_matrix_bool(text, ec), quiet)
    return "\n".join(
...
def make_qr_text(text: str, ec: str = "M", compact: bool = True, quiet: int = 4) -> str:
    if compact:
        return make_qr_text_compact(text, ec, quiet)
    return make_qr_text_full(text, ec, quiet)


def make_qr_matrix(text: str, ec: str = "M", quiet: int = 4) -> list[list[int]]:
    return _grid_with_quiet(_matrix_bool(text, ec), quiet)


def qr_text_size(
    text: str, ec: str = "M", quiet: int = 4, compact: bool = True
) -> tuple[int, int]:
    rows = _matrix_bool(text, ec)
    side = len(rows) + max(0, int(quiet)) * 2
    if compact:
        return side, (side + 1) // 2
    return side * 2, side


def make_qr_image(text: str, ec: str = "M", scale: int = 10, quiet: int = 4) -> bytes:
    try:
        from PIL import Image, ImageDraw
    except ImportError as exc:
        raise ImportError("make_qr_image requires Pillow: pip install Pillow") from exc
...
def rs_blocks(version, error_correction):
    ...
    return blocks
RS_BLOCK_OFFSET = {ERROR_CORRECT_L: 0, ERROR_CORRECT_M: 1, ERROR_CORRECT_Q: 2, ERROR_CORRECT_H: 3}
...
BIT_LIMIT_TABLE = [[0] + [8 * sum(map(_data_count, rs_blocks(version, error_correction))) for version in range(1, 41)] for error_correction in range(4)]
...
def create_data(version, error_correction, data_list):
    ...
    if len(buffer) > bit_limit:
        raise DataOverflowError('Code length overflow. Data size (%s) > size available (%s)' % (len(buffer), bit_limit))
...
class QRCode:
    ...
    def __init__(self, version=None, error_correction=ERROR_CORRECT_M, box_size=10, border=4):
...
_EC_MAP.update({"L": ERROR_CORRECT_L, "M": ERROR_CORRECT_M, "Q": ERROR_CORRECT_Q, "H": ERROR_CORRECT_H})
