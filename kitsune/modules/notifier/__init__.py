    ...
    async def start(self, token: str, *, first_run: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info("BotRunner: запускаем setup_all_avatars...")
                await setup_all_avatars(self._client, self._db)
                logger.info("BotRunner: setup_all_avatars завершён")
            except Exception as _ae:
                logger.warning("BotRunner: setup_all_avatars упал: %s", _ae, exc_info=True)
