...
def _make_bot(token: str) -> typing.Any:
    ...
    # AiohttpSession ожидает число секунд (int/float), а не ClientTimeout объект.
    # Внутри aiogram делает session.timeout + long_poll_timeout — оба должны быть числами.
    return Bot(
        token=str(token),
