"""
Unit-тесты для Phase 3 reliability subsystem.

Проверяем:
    • CircuitBreaker: CLOSED → OPEN после N провалов, OPEN → HALF_OPEN
      после cooldown, HALF_OPEN → CLOSED при успехе или OPEN при провале.
    • RetryWithBackoff: повторяет, считает попытки, бросает после maxAttempts.
    • DegradationFlags: установка/сброс флагов и to_dict().
...
class CircuitBreakerTests(unittest.TestCase):
    def setUp(self) -> None:
        # Уникальное имя на каждый тест чтобы реестр не делил состояние
...
class DegradationFlagsTests(unittest.TestCase):
    ...
    def test_global_flags_singleton(self) -> None:
        # Проверим что глобальный flags существует и имеет нужные атрибуты
...
class RegistryTests(unittest.TestCase):
    def test_get_breaker_idempotent(self) -> None:
        ...
        # Второй вызов не создаёт новый breaker, возвращает существующий
