"""Расширенные тесты для kitsune.crypto — покрытие AES-256-GCM ~100%."""
import sys, os
...
def isolate_key(monkeypatch, tmp_path):
    """Изолируем ключ от системных файлов: каждый тест получает чистую среду."""
...
def test_aes_gcm_tampered_ciphertext_fails():
    ...
    # ломаем последний байт (часть auth-тега)
    enc[-1] ^= 0xFF
...
def test_aes_gcm_tampered_nonce_fails():
    ...
    if not c._AES_GCM_AVAILABLE:
        pytest.skip("AES-GCM not available")
    enc = bytearray(c.encrypt(b"genuine"))
    # nonce начинается на смещении len(MAGIC)+len("AESGCM1:")+4
...
def test_aes_gcm_large_data():
    ...
    if not c._AES_GCM_AVAILABLE:
        pytest.skip("AES-GCM not available")
    big = os.urandom(1024 * 256)  # 256 KiB
    enc = c.encrypt(big)
