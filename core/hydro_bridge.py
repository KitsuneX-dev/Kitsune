...
class HydrogramBridge:
    ...
    def __init__(
        ...
        self._rl_times: deque = deque(maxlen=self._RL_MAX)
    def _rate_limit_ok(self) -> bool:
        if not self._rl_enabled:
            return True
        now = time.monotonic()
        while self._rl_times and now - self._rl_times[0] >= self._RL_WINDOW:
        ...
        try:
            ...
            async def _on_message(client, message) -> None:
                try:
                    ...
                    if _deg_flags.hydrogram_failed:
                        return
                except Exception:
                    pass
    ...
    async def _handle(self, hydro_msg) -> None:
        try:
            ...
            co_owners = (
                dispatcher._get_co_owners()
                if hasattr(dispatcher, "_get_co_owners")
                else self._db.get("kitsune.security", "co_owners", [])
            )
